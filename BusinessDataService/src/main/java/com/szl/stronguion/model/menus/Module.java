...
public class Module extends Model<Module> {
    ...
    public List<Module> getAdminModules(String parentid) {
        return dao.find("select * from eb_web_function_module where  parentid = '" + parentid + "' GROUP BY id;");
    }
    ...
    public List<Module> getNextModule(String parentid,int role_id,String level) {
        ...
        if (role_id==0){
            return dao.find("select * from eb_web_function_module where  parentid = '" + parentid + "' ORDER BY 'code';");
        }
        if (lev==1){
