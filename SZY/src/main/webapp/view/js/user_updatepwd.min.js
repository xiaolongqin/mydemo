...
$(function(){getUserInfo();new vfCode("J_vfCodeImg","J_vfCodeChange","updatepwd");});var REGCHECK={OLDPWD:{FOCUS:function(){_errorOldPwd.addClass("none");
...
}}}};function submitUpdatePwd(){var e=$.trim(_upOldPwd.val()),c=$.trim(_upPwd.val()),d=$.trim(_upPwdSure.val()),b=$.trim(_upVerification.val()),a=$("#J_errorUpPwd");
...
return false;}else{if(!/^[a-zA-Z0-9]{6}$/.test(b)){_errorVerification.removeClass("none").html("验证码不正确");return false;}}$.ajax({url:AJAXURL.updatePwd,data:{newpwd:c,input:b,captcha:"updatepwd"},type:"POST",dataType:"JSON",success:function(f){if(f.state){location.href="updatepwdrlt";
}else{if(f.msg==null){a.removeClass("none").html("数据异常，请稍后再试");new vfCode("J_vfCodeImg","J_vfCodeChange","updatepwd");}else{a.removeClass("none").html(f.msg);
new vfCode("J_vfCodeImg","J_vfCodeChange","updatepwd");}}},error:function(){a.removeClass("none").html("链接异常，请稍后再试");}});}function getUserInfo(){var b=$("#J_upPwdEmail"),a=$("#J_upPwdUserName");
$.ajax({url:AJAXURL.getUserInfo,type:"POST",dataType:"JSON",success:function(d){var c=d.data;if(d.state){b.html(c.email);a.html(c.name);}},error:function(){}});
}